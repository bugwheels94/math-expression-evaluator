...
describe('Testing Unit', function () {
	...
	it('check for constant inside Sigma', function () {
		assert.equal(
			a
				.lex('Sigma1,3,2', [{ type: 3, value: 'x', show: 'x', token: 'x' }])
				.toPostfix()
				.postfixEval({ x: 2 }),
			6
		)
	...
	it('check when two parenthesis less functions are consecutive on one parameter', function () {
		...
		assert.equal(a.lex('sinint2').toPostfix().postfixEval(), a.lex('sin(int(2))').toPostfix().postfixEval())
	})
	...
	it('check eval method with two argument when second one is value of constants', function () {
		a.addToken([{ type: 3, show: 'mexp', value: 'mexp', token: 'mexp' }])
	...
	it('check eval method with two argument when second one is value of constants', function () {
		...
		assert.equal(a.lex('mexp3').toPostfix().postfixEval(), '30')
	})
